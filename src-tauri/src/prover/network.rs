...
fn load_keypair(nockchain_dir: PathBuf, name: String) -> Result<Keypair, String> {
    ...
    if keypair_path.try_exists().map_err(|e| e.to_string())? {
        ...
        let keypair =
            libp2p::identity::Keypair::from_protobuf_encoding(&keypair_bytes[..])
            .map_err(|e| e.to_string())?;
        ...
        let new_keypair_bytes = 
            new_keypair.to_protobuf_encoding()
            .map_err(|e| e.to_string())?;
...
fn make_limits() -> Result<connection_limits::ConnectionLimits, String> {
    let libp2p_config = 
        nockchain_libp2p_io::config::LibP2PConfig::from_env()
        .map_err(|e| e.to_string())?;
